			...
			*/
			...
			if (config.use_hash == true) {
				...
				if (is_pushstate_supported) {
					$(window).on('popstate', function(e) {
						...
						var full_popup = etoos_util.getPageParamValue('full-popup').toLowerCase(); // 전체화면 팝업 실행여부, 실행 시 "open"값 반환
						var alert_popup = etoos_util.getPageParamValue('alert-popup').toLowerCase(); // 알럿형 팝업 실행여부, 실행 시 "open"값 반환
						var header_menu = etoos_util.getPageParamValue('header-menu').toLowerCase(); // 헤더메뉴 실행여부, 실행 시 "open"값 반환
			...
			if ("onorientationchange" in window) {
				...
				$(window).on('resize', function(e) {
					if (etoos_util.getOSName() != "unknown") {
	...
	*/
	context.loadMenuContentComplete = function(menu) {
		...
		if (config.use_hash == true) {
			...
			if (!etoos_util.isNumeric(page_scroll_top)) {
				page_scroll_top = "";
			}
		}
			...
			setTimeout(function() {				
				...
				if (anchor != "") {
					if ($('#'+ anchor).length > 0) {
						...
						if (etoos_util.isNumeric(anchor_rev_px)) {
