...
function initEtoosUI() {
	...
	$('body').on('click', '.viewbtn', function () { /** 수정 : 이벤트 객체대상 body 로 확장 ***/
		if ($(this).hasClass("on")) {
			...
			$('.board_view').slideDown(400, function () {
				if (etoos_swiper) {
					etoos_swiper.resizeContentHeight();
				}
			});
		} else {
	...
	if (is_pushstate_supported) {
		...
		$(window).on('popstate:ui', function (e) {
			...
			// 전체화면 팝업관련 : 시작
			var full_popup = etoos_util.getPageParamValue('full-popup');
			...
			if (full_popup == 'open') { // 전체화면 팝업 열기
				if (!is_full_popup_open) {
					...
					if (window.history.state) {
						...
						var pop_obj = etoos_util.getPageParamValue('pop-obj');
						var pop_url = decodeURIComponent(etoos_util.getPageParamValue('pop-url'));
						var anim_type = etoos_util.getPageParamValue('anim-type');
			...
			// 알럿 팝업관련 : 시작
			var alert_popup = etoos_util.getPageParamValue('alert-popup');
			...
			if (alert_popup == 'open') { // 알럿 팝업 열기
				if (!is_alert_popup_open) {
					...
					if (window.history.state) { // state값이 있다면..
						...
						var alert_pop_obj = etoos_util.getPageParamValue('alert-pop-obj'); // 팝업 object 값이 있다면..
	...
	if (pop_type == 'full') { // 전체화면 팝업
		...
		if (target) {
			if (is_pushstate_supported) {
				...
				var set_url = etoos_util.pageParamMake(null, null, 'full-popup', 'open', ['full-popup', 'pop-obj', 'pop-url', 'anim-type']);
		...
		if (pop_obj != undefined && pop_obj != null && pop_obj != "") {
			if (is_pushstate_supported) {
				...
				var set_url = etoos_util.pageParamMake(null, null, 'alert-popup', 'open', ['alert-popup', 'alert-pop-obj']);
...
function fnPopupClose(pop_type, pop_obj, is_replace) {
	if (pop_type == 'full') { // 전체화면 팝업
		if (is_pushstate_supported) {
			var set_url = etoos_util.pageParamMake(null, null, null, null, ['full-popup', 'pop-obj', 'pop-url', 'anim-type']);
		...
		if (is_pushstate_supported) {
			...
			var set_url = etoos_util.pageParamMake(null, null, null, null, ['alert-popup', 'alert-pop-obj']);
...
function mainOnNav() {
	if (is_pushstate_supported) {
		...
		if (is_header_menu_open) {
			...
			set_url = etoos_util.pageParamMake(null, 'header-menu');
			...
			set_url = etoos_util.pageParamMake(null, null, 'header-menu', 'open');
